...
- CSS variable syntax (`rgb(var(--color-pitch) / 1)`) works in stylesheets but NOT in React `style={}` prop values — `var()` is not resolved by JavaScript. Use hex literals for inline styles. Pirate palette: pitch `#150c0c`, coal `#2a1a1a`, mahogany `#3d1f1f`, blood `#8b0000`, blood-bright `#c41e3a`, gold `#d4a040`, bone `#e8dcc8`, parchment `#f0e6d3`, copper `#b87333`.
...
- **Server route tests:** Create an Express app in `beforeAll`, mount the router, bind to port 0 (dynamic) with `server.listen(0, '127.0.0.1', resolve)`, use Node 20 built-in `fetch` for requests. Cast `server.address() as AddressInfo` from `net` to get the port. Mock Prisma via `vi.hoisted()` + `vi.mock('../services/db.ts')`. For modules with a module-level `setInterval` (e.g., any service module with a module-level `setInterval`), mock the entire module to prevent background timers firing during tests. For Prisma `$transaction` mocks: two forms exist — (a) **callback form** `prisma.$transaction(async (tx) => { ... })` used when the route needs a return value from the transaction: mock as `$transaction: vi.fn().mockImplementation((fn) => fn({ user: txUser, ... }))` where `txUser` is a separate transaction-context mock object; (b) **array form** `prisma.$transaction([op1, op2])` used for batched ops with no result needed: mock as `$transaction: vi.fn().mockImplementation(async (ops) => Promise.all(ops))`. Using the wrong mock for the wrong form fails silently. Throwing from the callback propagates as a rejected promise for the route's outer catch. After `vi.clearAllMocks()` in `beforeEach`, restore the `$transaction` default implementation explicitly — `clearAllMocks()` wipes `mockImplementation` and tests that don't re-mock will fail silently. When a route migrates to use `$transaction`, all existing success tests that mock `prisma.user.create` must be updated to mock the transaction-context `tx.user.create` instead.
- **Replacing data in shared modules:** When changing values in a shared module (mission titles, captain names, enum values), grep for hard-coded strings across ALL test files: `grep -r "old-value" shared/src/__tests__/ client/src/__tests__/` — client store tests can import shared data and hard-code expected strings; missing these causes silent failures.
- **Env-var-dependent route tests:** Use `vi.stubEnv('VAR_NAME', 'value')` to inject env vars and `vi.unstubAllEnvs()` in `afterEach` to restore. Import `afterEach` from `vitest` explicitly.
- **Prisma compound unique where clauses:** `@@unique([userId, achievementId])` in schema generates where clause name `userId_achievementId`. Use `where: { userId_achievementId: { userId, achievementId } }` in upsert/findUnique. Format: `<field1>_<field2>` joined with underscore.
...
- When server auth routes add new user fields to JWT payload or `/auth/me` response, also update the `User` interface in `client/src/store/authStore.ts` in the same PR. New fields must be optional (`field?: type`) — existing localStorage sessions won't have them.
